...
# Load configs
source "$PROJECT_ROOT/configs/model/flame-moe.sh"
...
# Activation checkpointing (if enabled via environment)
# Note: --recompute-activations forces granularity=selective and conflicts with --recompute-method
# So we only use --recompute-granularity + --recompute-method for full recompute
RECOMPUTE_ARGS=()
if [ "${ENABLE_RECOMPUTE:-0}" = "1" ]; then
    ...
    if [ -n "${RECOMPUTE_METHOD:-}" ]; then
        RECOMPUTE_ARGS+=(--recompute-method "$RECOMPUTE_METHOD")
    fi
    if [ -n "${RECOMPUTE_NUM_LAYERS:-}" ]; then
        RECOMPUTE_ARGS+=(--recompute-num-layers "$RECOMPUTE_NUM_LAYERS")
    fi
    echo "Activation checkpointing enabled: ${RECOMPUTE_ARGS[*]}"
fi
