...
from megatron.core.transformer.moe.moe_layer import MoPELayer  # added for MoPE support
...
class MoEOrMoPE(MegatronModule):
    ...
    def __init__(self, config, submodules=None, layer_number=None):
        # Important: pass config to parent MegatronModule
        super().__init__(config=config)

        use_mope = getattr(config, "moe_router_load_balancing_type", "") == "otep"
        cls = MoPELayer if use_mope else MoELayer
        # Allow layer_number to be None initially; Megatron will call set_layer_number later
